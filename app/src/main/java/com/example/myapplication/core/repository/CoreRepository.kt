...
class CoreRepository(private val remoteDataSource: RemoteDataSource) {
        ...
        try {
            remoteDataSource.login(data).let {
                if (it.isSuccessful) {
                    ...
                    emit(
                        Resource.error(
                            it.getErrorBody(LoginRespon::class.java)?.message ?: "",
                            null
                        )
                    )
                }
    ...
    fun book(token: String) = flow {
        emit(Resource.loading(null))
        try {
            remoteDataSource.book(token).let {
                if (it.isSuccessful) {
                    val body = it.body()
                    val user = body?.data
                    emit(Resource.success(user))
